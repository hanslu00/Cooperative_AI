...
if __name__ == '__main__':
    ...
    SAVE_PATH = f"dyn_agent1/ppo1"

    blue_networks = [PPONet(n_observations, n_actions), PPONet(n_observations, n_actions)]
    # load_state_dicts_for_team_required_grad_true(blue_networks, ["agent1/ppo1/actor_A10.pth", "agent1/ppo1/actor_B10.pth"], ["agent1/ppo1/critic_A10.pth", "agent1/ppo1/critic_B10.pth"])

    
    print("STARTING")
    channel = EngineConfigurationChannel() 
    env = UnityEnvironment(file_name="../dynamic_environment/DynamicEnvironment1", seed=args.seed, side_channels=[channel], worker_id = args.worker_id)
    ...
    print("Playing with versus random agents")
    for ep in range(1, n_episodes+1):
        ...
        # for i in range(2):
            b_observations = get_observations(env, team_blue)
            b_actions, b_logprobs, b_vals = set_action_for_team(env, team_blue, blue_networks, b_observations)
            set_random_action_for_team(env, team_purple)
            env.step()
            simulation_steps += 1
            ...
            for ind in range(2):
                for blue_net in blue_networks:
                    blue_net.store(b_observations[ind], b_actions[ind], b_logprobs[ind],
