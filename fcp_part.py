...
if __name__ == '__main__':
    state_dicts = []
    critic_dicts = []
    SAVE_PATH = f"agent2/fcp"
    
    print("STARTING")
    channel = EngineConfigurationChannel() 
    # env = UnityEnvironment(file_name="../Soccer2v2_thingy", seed=args.seed, side_channels=[channel], worker_id = args.worker_id)
    env = UnityEnvironment(file_name="../Soccer2v2_thingy", seed=args.seed, side_channels=[channel], worker_id = 367)
    ...
    pro_critic = torch.load("agent2/ppo2/critic_B6.pth")
    pro = torch.load("agent2/ppo2/actor_B6.pth")
    intermediate = torch.load("agent2/ppo1/actor_A10.pth")
    beginner = torch.load("agent2/ppo1/actor_A1.pth")
